    ...
    try:
        if cv2.cuda.getCudaEnabledDeviceCount() > 0:
            ...
            if net.getPreferableBackend() != cv2.dnn.DNN_BACKEND_CUDA:
                gpu_status = "⚠ CUDA not available - Using CPU"
        else:
            gpu_status = "⚠ No CUDA devices found - Using CPU"
    except Exception:
        gpu_status = "⚠ CUDA not supported in this OpenCV build - Using CPU"
...
def send_telegram_alert(bot_token=None, chat_id=None, result_queue=None, alert_type="presence", count=0, duration=0):
    ...
    try:
        if alert_type == "absence":
            ...
            message = "⚠ Workforce monitoring alert"
    ...
    while run_camera:
        ...
        if absence_start_time is not None:
            ...
            if (absence_duration >= st.session_state.absence_threshold and 
                ...
                if bot_token and chat_id:
                    ...
                    st.toast(f"⚠ No workers detected for {int(absence_duration)} minutes!")
                else:
        ...
        if (count < st.session_state.alert_threshold and 
            ...
            if bot_token and chat_id:
                ...
                st.session_state.alert_queues.append(alert_queue)
                st.session_state.last_alert_time = time.time()
                st.toast(f"⚠ Only {count} workers detected (minimum is {st.session_state.alert_threshold})")
            else:
                st.warning("Telegram credentials missing")
...
with st.expander("ℹ About this App & Usage Guide", expanded=False):
    st.markdown("""
